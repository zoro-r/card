  ...
  static async getPaymentStats(ctx: Context) {
    try {
      ...
      const [
        ...
        // 总记录数
        WechatPayment.countDocuments(query),
        
        // 总金额统计（只统计已支付的）
        WechatPayment.aggregate([
          { $match: { ...query, status: 'PAID' } },
          { $group: { _id: null, totalAmount: { $sum: '$totalFee' } } }
        ]),
      ...
      statusStatsResult.forEach((item: any) => {
      ...
      ctx.body = success({
        total,
        totalAmount,
        totalAmountYuan: parseFloat(totalAmountYuan),
        statusStats
      }, '获取支付统计成功');
    } catch (error) {
      console.error('获取支付统计失败:', error);
      ctx.body = fail('获取支付统计失败');
    }
  }
}